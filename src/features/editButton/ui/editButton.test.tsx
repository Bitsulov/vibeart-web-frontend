...
describe("EditButton - кнопка редактирования поста", () => {
    it("Рендерится как ссылка", () => {
        renderWithProviders(<EditButton UUID={POST_UUID} type="post" ariaLabel="Редактировать пост" />);
    ...
    it("Вызывает onClick и onMouseLeave при клике", () => {
        ...
        renderWithProviders(
            <EditButton UUID={POST_UUID} type="post" ariaLabel="Редактировать" onClick={onClick} onMouseLeave={onMouseLeave} />
        );
    ...
    it("Принимает дополнительный className", () => {
        renderWithProviders(<EditButton UUID={POST_UUID} type="post" ariaLabel="Редактировать" className="extra" />);
