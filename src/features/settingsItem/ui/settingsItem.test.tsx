...
vi.mock("../model/deleteButtonClickHandler", () => ({
    deleteButtonClickHandler: vi.fn()
}));
vi.mock("../model/onChangeAvatarLoadHandler", () => ({
    onChangeAvatarLoadHandler: vi.fn()
}));
vi.mock("../model/deleteAlbumButtonClickHandler", () => ({
    deleteAlbumButtonClickHandler: vi.fn()
}));
...
describe("SettingsItem - универсальный элемент настроек", () => {
    ...
    describe("Загрузка и удаление изображения", () => {
        it("Отображает кнопки загрузки и удаления", () => {
            ...
            expect(
                screen.getByRole("button", { name: "ariaLabel.loadImg" })
            ).toBeInTheDocument();
            expect(
                screen.getByRole("button", { name: "ariaLabel.deleteImg" })
            ).toBeInTheDocument();
        });
    ...
    describe("Редактирование аватара", () => {
        ...
        it("Отображает заглушку если аватар не задан", () => {
            renderWithProviders(
                <SettingsItem {...baseProps} type="avatar" avatarUrl="" />
            );
