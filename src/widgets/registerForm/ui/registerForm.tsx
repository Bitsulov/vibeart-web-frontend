...
export const RegisterForm = ({ ...props }) => {
    ...
    const {register, reset, setValue, handleSubmit, control, formState: {errors, isSubmitted}, setError} = useForm<IRegisterForm>({
    ...
    const registerMutation = useMutation({
        ...
        onSuccess: (response, request) =>
            registerSuccessHandler(response, request, dispatch, setIsEmailSent, setValue, queryClient, setSentEmail),
        onError: (error: AxiosError<AppError>) =>
            registerErrorHandler(error, setError)
    });
            ...
            <div className="container">
                ...
                :
                    ...
                    >
                        ...
                        <StylizedButton
                            type="button"
                            onClick={() => returnToRegisterHandler(setIsEmailSent, codeForm.reset)}
                            aria-label={t("ariaLabel.backToRegister")}
                            className={c.back}
                        >
                            <ArrowLeft className={c.icon} width="15" height="15" />
                        </StylizedButton>
                        <p className={c.text}>{t("register.sentCodeText", {address: sentEmail})}</p>
                        <CodeInputs
                            isError={codeError}
                        ...
                        <button
                            ...
                            onClick={() =>
                                sentCodeButtonHandler(setIsAllowSentCode, setTimer, sentEmail, sendCodeMutation.mutateAsync)
                            }
                            className={clsx(c.send_code, !isAllowSentCode && c.disabled)}
                        >
                            {t(sendButtonText, {timer: timer})}
                        </button>
                        <StylizedButton aria-label={t("ariaLabel.confirmRegister")} type="submit" className={c.submit_code}>
                            {t("Send")}
                        </StylizedButton>
                    </form>
                }
