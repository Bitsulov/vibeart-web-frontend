...
test.describe("Contacts - страница связи с администрацией", () => {
    ...
    test("Отображается описание с email", async ({page}) => {
        ...
        await expect(page.getByRole("main").getByText(/vibeartfake@mail\.ru/)).toBeVisible();
    });
    ...
    test("Отображается кнопка отправки", async ({page}) => {
        ...
        await expect(page.getByRole("button", {name: "Send a report to the administration"})).toBeVisible();
    });
    ...
    test("Пустая отправка показывает уведомление об ошибке", async ({page}) => {
        ...
        await page.getByRole("button", {name: "Send a report to the administration"}).click();

        await expect(page.getByText("Enter message text")).toBeVisible();
    });

    test("Сообщение длиннее 1000 символов показывает уведомление об ошибке", async ({page}) => {
        await page.goto(CONTACTS_URL);
    ...
    test("Успешная отправка показывает уведомление и сбрасывает поле", async ({page}) => {
        await page.goto(CONTACTS_URL);
